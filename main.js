...
let container = [], firstCard = null, secondCard = null, completeCards = [], seconds = 0, timerInterval=null;
...
pG.addEventListener('click',(e)=>{
  if(firstCard == null && e.target.classList.contains('card') && !e.target.classList.contains('active') && !e.target.classList.contains('completed')){
    e.target.classList.add('active');
    firstCard = e.target;
  } else if(secondCard == null && e.target.classList.contains('card') && !e.target.classList.contains('active') && !e.target.classList.contains('completed')) {
    e.target.classList.add('active');
    secondCard = e.target;
...
function createRecordLi(){
  ...
  if(!item[0].search(/record/i)) {
    ...
    recordLi.textContent = `Рекорд ${+item[0].replace(/\D/ig,'')}x${+item[0].replace(/\D/ig,'')}: ${item[1]} с.`;
...
function checkRecord(sec){
  if(!localStorage.getItem(`record${input.value}`)) {localStorage.setItem(`record${input.value}`, sec.toFixed(2)); createRecordLi();}
